...
struct mat {
	...
	*/
	void operator()(this auto&& self, int64_t r, int64_t c, std::vector<uint8_t> rgb) {
		for (int64_t i = 0; i < 3; ++i) {
			self.data_[r * self.cols_ + c + i] = rgb[i];
		}
	}
...
void mirror(mat<uint8_t> img, std::ifstream& is, std::ofstream& os) {
	...
	for (int64_t r = 0; r < img.rows(); ++r) {
		for (int64_t c = img.cols() - 1; c >= 0; --c) {
			is.get(ch);
